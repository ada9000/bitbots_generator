...
OUTPUT_DIR = "../output/"
META = "nft-meta.json"
JSON_OUT = "nft-data.json"
WEIGHTS_OUT = "nft-weights.json"
...
# TODO PUT HALF THIS IN A JSON FILE
...
if __name__ == "__main__":
    # gen meta and 
    meta, nft_data = generate_meta()
    # size of string calcs for rough checks
    json_str = json.dumps(nft_data, ensure_ascii=False, indent=4)
    ...
    # update weights
    w = update_weights(nft_data, total)

    # write json to file
    if REFRESH_META:
        write_json(JSON_OUT, nft_data)

    clean()
    nfts = weighted_rand(w)

    nfts_to_svgs(nft_data, nfts)
